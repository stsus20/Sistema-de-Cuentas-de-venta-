...
function renderHistory() {
  $("#historyBody").innerHTML = historyClients
    .map(
      (client) =>
...
function switchView(view) {
  ...
  $("#clientsView").classList.toggle("hidden", history);
  $("#historyView").classList.toggle("hidden", !history);
  $("#clientsNav").classList.toggle("nav-active", !history);
  $("#historyNav").classList.toggle("nav-active", history);
  const target = history ? $("#historyView") : $("#clientsView");
  target.classList.remove("view-enter");
  requestAnimationFrame(() => target.classList.add("view-enter"));
  if (history) loadHistory($("#historySearch").value);
  else loadClients($("#searchInput").value);
}
...
$("#movementForm").addEventListener("submit", async (event) => {
  event.preventDefault();
  const values = Object.fromEntries(new FormData(event.currentTarget));
  if (movementType === "purchase" && (!values.producto || !values.precio))
    return notify("Completa el producto y el precio.", true);
